...
function metricTable (endpoint, gridOptions, queryParams, options, screen, metricUnits) {
  ...
  var labels = (this.dimensions + ',' + this.metrics + ',node').split(',')
  options.columnWidth = labels.map(label => label.length + 12)
  ...
  dataGenerator.getMetricData(metricTable.endpoint, metricTable.metrics, metricTable.aggregates, metricTable.dimensions,
    function (metricData) {
      ...
      if (Object.keys(aggregatedData).length === 0) {
        ...
        callback({ 'headers': appendMetricUnits(aggregatedData.dimensions, metricTable.metricUnits),
