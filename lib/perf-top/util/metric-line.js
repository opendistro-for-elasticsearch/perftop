...
function metricLine (endpoint, gridOptions, queryParams, options, screen) {
  ...
  var grid = new contrib.grid({ rows: gridOptions.rows, cols: gridOptions.cols, screen: screen }); // /set grid 
  this.line = grid.set(options.gridPosition.row, options.gridPosition.col, options.gridPosition.rowSpan, // /set position of the line
 ...
 * @param {object} screen - blessed.screen() object.
 */
metricLine.prototype.generateGraph = function (line, screen) { // /Called by metric-graph.start()
...
function generateAndUpdateLineData (metricLine) {
  dataGenerator.getMetricData(metricLine.endpoint, metricLine.metrics, metricLine.aggregates, metricLine.dimensions,
    function (metricData) { 
      ...
      var lineNames = Object.keys(metricData); // /lineNames = keys of metricData = {nodeNames}
      ...
      for (var i = 0; i < lineNames.length; i++) {
        var lineName = lineNames[i]; // /key
        var lineData = metricData[lineName]; // /values
...
//TODO generateAndUpdateLineData for Temperature Analsis
