...
function getMetricData(endpoint, metrics, aggregates, dimensions, done) {
  ...
  var urlOptions = getURLOptions(
    endpoint,
    `/_opendistro/_performanceanalyzer/metrics?${metricParam}${aggParam}${dimParam}&nodes=all`
  );
 ...
 * @param {function(object):void} done - callback to be called when the response is parsed into a hashmap object.
 */
function getMetricDataTemperatureAnalysis(endpoint, name, local, dimension, graphParams, type, done) {
  ...
  makeRequest(urlOptions, function (response) {
    if (response === "") {
      console.error("empty response from server");
        ...
        getDataPerNodeTemperatureAnalysis(
          response,
          dimension,
          graphParams,
          type
...
function getMetricUnits(endpoint, done) {
  var urlOptions = getURLOptions(
    endpoint,
    "/_opendistro/_performanceanalyzer/metrics/units"
  );
  makeRequest(urlOptions, function (response) {
    if (response === "") {
      console.error(
...
function addCommaDelimiter(data) {
  ...
  for (var column = 0; column < numColumn; column++) {
    var dataColumn = data.map(function (row, y) {
      return row[column];
    });
    if (
      dataColumn.every(function (dataValue) {
 ...
 *
 * @param {object} metricData - hashmap object in the format of  // /allData
...
function getDataPerNode(rawData) {
  var jsonData = {};
  try {
    jsonData = JSON.parse(rawData); // / parse rawData in String form to JSON format: jsonData
    if (Object.keys(jsonData).length === 1 && "error" in jsonData) {
  ...
  for (var nodeName in jsonData) {
    ...
    if (
      !("fields" in jsonData[nodeName].data) &&
      !("records" in jsonData[nodeName].data)
    ) {
      console.error(`Data returned for nodeName=${nodeName} was in an unexpected format:
        ${JSON.stringify(jsonData[nodeName])}`);
      continue;
    ...
    allData[nodeName] = {
      ...
      data: nodeData,
      timestamp: jsonData[nodeName].timestamp,
    };
  }
  return allData;
}
...
function getDataPerNodeTemperatureAnalysis(rawData, dimension, graphParams, type) {
  var jsonData = {};
  try {
    jsonData = JSON.parse(rawData).AllTemperatureDimensions[0]
...
function getNodeData(metricData, nodeName) {
  var node = Object.keys(metricData).filter((lineName) =>
    lineName.startsWith(nodeName)
  );
  var nodeData = {};
  if (node.length === 0) {
...
function getDimensionData(metricData, dimensionFilters) {
  for (var nodeName in metricData) {
    ...
    var dimensionData = metricData[nodeName].data.filter((data) =>
      dimensionFilters.includes(data[0])
    );
...
function removeStaleData(metricData, dataTimestamp) {
  for (var nodeName in metricData) {
    ...
    if (nodeName in dataTimestamp) {
      if (metricData[nodeName].timestamp > dataTimestamp[nodeName].timestamp) {
        dataTimestamp[nodeName] = {
          counter: 0,
          timestamp: metricData[nodeName].timestamp,
        };
      } else {
    ...
    if (dataTimestamp[nodeName].counter >= 3) {
      console.error(
        `Data for node '${nodeName}' has not been updated for ` +
 ...
 *                                        timestamp: timestamp } }
 */
function removeStaleDataTemperatureAnalysis(metricData, metricGraph) {
  ...
  // if (dataTimestamp[field].counter >= 3) { // uncomment this when the response becomes dynamic
...
module.exports.getMetricDataTemperatureAnalysis = getMetricDataTemperatureAnalysis;
...
module.exports.removeStaleDataTemperatureAnalysis = removeStaleDataTemperatureAnalysis;
